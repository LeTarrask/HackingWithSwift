...
class IntentHandler: INExtension, INRidesharingDomainHandling {
    ...
    // Methods that can't be faked about the ride, and we won't use in the tutorial, just to make XCode shut up.
    func handle(intent: INGetRideStatusIntent, completion: @escaping (INGetRideStatusIntentResponse) -> Void) {
        
        // This code just replies "everything's fine" when user asks about their ride.
